...
get_rules_html() {
    ...
    if [[ -z "$recent_rules_html" ]]; then
        recent_rules_html="<p style='color: var(--muted); font-size: 12px;'>No rules configured</p>"
    fi
...
handle_request() {
    ...
    if [[ -n "$post_data" ]]; then
        ...
        case "$action" in
            ...
            set_policy)
                if [[ -n "$policy" ]]; then
                    message="<div class='success'>✓ Policy set to: $policy</div>"
                    sudo ufw default "$policy" incoming 2>/dev/null
                fi
                ;;
            set_logging)
                if [[ -n "$level" ]]; then
                    message="<div class='success'>✓ Logging set to: $level</div>"
    ...
    # Get recent rules
    local recent_rules_html
    recent_rules_html=$(get_rules_html)
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .rule-port { color: var(--accent2); font-weight: 600; }
  .rule-action { color: var(--green); }
  .success, .error {
    padding: 10px;
    border-radius: 4px;
    margin-bottom: 12px;
...
<div class="container">
  ...
  <div class="grid">
    <div class="card">
      ...
      <div class="stat-row">
        <span class="stat-label">UFW:</span>
        <span class="stat-value">$ufw_status</span>
      </div>
      <div class="stat-row">
      ...
      <div class="stat-row">
        ...
        <span class="stat-value">$logging_level</span>
      </div>
    </div>
