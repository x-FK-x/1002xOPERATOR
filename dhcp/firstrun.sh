...
if [ -z "$LAN_INTERFACE" ]; then
    error "LAN interface not detected."
    exit 1
fi
...
echo "=== DHCP Check Script ==="
echo "Do you want to control the settings?"
read -p "Type YES to confirm: " CONFIRM

if [[ "$CONFIRM" != "YES" ]]; then
    echo "[INFO] Check cancelled."
    exit 0
fi

bash /etc/1002xOPERATOR/dhcp/check.sh
